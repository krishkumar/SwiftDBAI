...
public struct ChatEngineConfiguration: Sendable {
    ...
    /// When enabled, uses AnyLanguageModel's `@Generable` struct to request
    /// structured JSON output instead of free-form text. This eliminates
    /// SQL parsing issues (no markdown fences, no backtick stripping) but
    /// requires the LLM to support structured/constrained generation.
    ///
    /// Default is `false` (uses text parsing). Enable when using models
    /// that support structured output well (e.g. GPT-4o, Claude, Gemini).
